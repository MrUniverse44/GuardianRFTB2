...
public class BeastSpawnRunnable extends BukkitRunnable {
    ...
    @Override
    public void run() {
        if(!currentGame.getBeasts().isEmpty() && !currentGame.getRunners().isEmpty() && currentGame.getStatus().equals(GameStatus.PLAYING)) {
            ...
            if(time != 0) {
                ...
                for (Player player : PlayerUtil.getPlayers(plugin, currentGame.getBeasts())) {
                    plugin.getGamePlayer(player).setScoreboard(PluginScoreboard.PLAYING);
            ...
            if(currentGame.getBeasts().isEmpty()) {
